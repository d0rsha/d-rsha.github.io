...
void AVL_Tree_Node<Comparable>::remove(const Comparable& x, Node_Pointer& t)
{
   ...
   {
      // Sökt värde finns i noden t; Ska tas bort h�r 
      Node_Pointer  tmp;
      ...
      if (t->left != nullptr && t->right != nullptr) {
         ...
         if (t->left == nullptr)
            t = t->right;  // Det kvittar om (t->right == nullptr) 
         else
            t = t->left;
   ...
   if(t != nullptr)
   {
      //Kontrollera h�jdskillnad till h�ger
      if (node_height(t->right) - node_height(t->left) == 2)
      {
         if (node_height(t->right->right) > node_height(t->right->left))
            ...
            double_rotate_with_right_child(t);
      // Uppdaterar höjden i t EFTER rotation
      calculate_height(t); 
      }

      // Kontrollera h�jdskillnad till v�nster
      if (node_height(t->left) - node_height(t->right) == 2)
      {
